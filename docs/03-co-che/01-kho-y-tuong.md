...
Kho brainstorm **mọi ý tưởng cơ chế** quanh lõi (trọng lực xoay · vật lý cụm cứng · block-fit · cụm có số/màu). KHÔNG phải tất cả sẽ dùng — đây là pool để lọc. Đã có riêng: **merge-9 → siêu khối** (`../02-thiet-ke-man/01-schema-man.md`).
...
- **A8. Hồi ngân sách xoay theo combo — ✅ CHỐT, áp dụng MỌI cách chơi.** Mỗi lần **bậc combo tăng lên mức ≥ x2** → hồi **+1 lượt xoay** (hồi cộng dồn = bậc − 1). Vòng thưởng dùng đúng cơ chế chữ ký. Cân nhắc trần hồi/chặng để tránh xoay vô hạn. Là hàm thuần của chuỗi combo → giữ deterministic. Neo: chữ ký + combo. Luật đầy đủ: `../01-nen-tang/01-nghiep-vu.md §6` + `02-combo.md §4`.
...
- **04/07/2026** — A8: bỏ phần liệt kê per-tier trùng lặp `(x2=+1, x3=…)`, giữ luật gọn `hồi cộng dồn = bậc − 1` và trỏ luật đầy đủ về `../01-nen-tang/01-nghiep-vu.md §6` + `02-combo.md §4`.
