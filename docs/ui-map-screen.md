...
Tham chiếu: `level-design.md` (mô hình màn, gate sao, biome), `level-system-infinite.xlsx`. Sau màn 100, nội dung mở dần ở các bản sau — 1.0.0 chỉ cần 100 màn này hoàn chỉnh.
...
- Mỗi **chương = world** trong 100 màn đầu (đơn giản hoá cho 1.0.0; mô hình chương 15-màn ở `level-design.md` áp dụng cho giai đoạn vô hạn về sau).
...
**Chuyển cảnh nền:** nền parallax đổi biome dần khi cuộn qua ranh giới (blend mềm), không cắt cứng. Mỗi world một palette (theo `level-design.md`): Đồng cỏ → Rừng → Sông → Sa mạc → Biển → Núi tuyết → Hang băng → Núi lửa → Bầu trời → Vũ trụ.
...
Map có thể dài 100+ node — phải tuân checklist ở `technical-approach.md`:
