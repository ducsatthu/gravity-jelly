...
Bám sát cơ chế cốt lõi ở `business-understanding.md` (xoay trọng lực, vật lý cụm cứng, combo, ngân sách xoay hữu hạn) và yêu cầu deterministic + kiến trúc 3 lớp ở `technical-approach.md`.

Bảng dữ liệu kèm theo (cùng thư mục):

- `level-system-infinite.xlsx` — **mô hình vô hạn** (đường cong tới ~5000 màn, vòng cảnh, modifier pool, goal cycle). Đây là tài liệu chính.
- `level-design-100-levels.xlsx` — bộ 100 màn cụ thể, nay đóng vai **"loop onboarding" (vòng ribbon đầu tiên)**, không còn là toàn bộ game.
...
*Bảng dữ liệu: `level-system-infinite.xlsx` (4 sheet: Đường cong vô hạn · Vòng cảnh (loop) · Modifier pool · Goal cycle). Bộ 100 màn onboarding chi tiết: `level-design-100-levels.xlsx`.*
