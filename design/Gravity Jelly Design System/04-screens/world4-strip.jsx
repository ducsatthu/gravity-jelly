...
(function () {
  ...
  function Scene() {
    return (
      <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`}
        ...
        {/* ── Top band: World 5 Bãi biển — NHÌN TỪ TRÊN. BỜ CÁT rộng; BIỂN
           CHỈ Ở BÊN PHẢI con đường (vũng nước góc phải-trên). Con đường đi
           trên cát, bên trái hoàn toàn là bãi cát. ── */}
        <rect x="0" y="-20" width="360" height="318" fill="#F1DEAF" />
        {/* vũng biển — chỉ bên PHẢI đường */}
        <path d="M205 -20 C 206 34, 222 74, 272 100 C 312 118, 350 128, 380 134 L 380 -20 Z" fill="#5BB4CB" />
        <path d="M205 -20 C 206 34, 222 74, 272 100 C 312 118, 350 128, 380 134 L 380 -20 Z" fill="url(#w4s-oasis)" opacity="0.55" />
        {/* bọt sóng mép bờ (ranh giới cát ↔ biển) */}
        <path d="M205 -20 C 206 34, 222 74, 272 100 C 312 118, 350 128, 380 134" fill="none" stroke="#FFFFFF" strokeWidth="3" opacity="0.75" strokeLinecap="round" />
        <path d="M213 -20 C 214 36, 230 76, 280 102 C 318 120, 354 130, 380 136" fill="none" stroke="#CFEAF2" strokeWidth="2" opacity="0.6" strokeLinecap="round" />
        {/* gợn sóng nhỏ trong vũng (phải) */}
        {[[300,22],[260,46],[338,40],[286,76],[344,86],[314,104]].map(([x,y],i)=>{
          const s = 6 + (i % 2) * 2;
          return <path key={`cr${i}`} d={`M ${x-s} ${y} q ${s} ${s*0.8} ${2*s} 0`}
                       fill="none" stroke="#FFFFFF" strokeWidth="1.6" opacity="0.5" strokeLinecap="round" />;
        })}
        {/* vài gợn cát mịn trên bãi (trái + dưới) */}
        {[[90,150],[60,196],[140,200]].map(([x,y],i)=>(
          <path key={`sd${i}`} d={`M ${x-26} ${y} q 26 -5 52 0`} fill="none" stroke="#E6CF9E" strokeWidth="2" opacity="0.5" strokeLinecap="round" />
        ))}
        {/* beach palms (top-down) + shells trên BÃI CÁT — KHÔNG mặt trời */}
        <Palm x={70}  y={130} s={28} />
        <Palm x={118} y={200} s={24} />
        <Palm x={300} y={196} s={22} />
        <Rock x={150} y={206} r={12} c="#E7D3A8" edge="#C9B07E" />
        {[[100,158],[58,212],[176,206],[326,168]].map(([x,y],i)=>(
          <Shell key={i} x={x} y={y} />
        ))}
        ...
        {/* ── Vạt cát sáng (ánh nắng trên cát) — KHÔNG vẽ đĩa mặt trời ── */}
        <circle cx="270" cy="640" r="130" fill="url(#w4s-sun)" opacity="0.6" />
  ...
  function Palm({ x, y, s = 28 }) {
    // cây cọ NHÌN TỪ TRÊN XUỐNG: tán lá tỏa tròn quanh thân, 2 lớp cho đầy
    const dirs = [0, 45, 90, 135, 180, 225, 270, 315];
    return (
      <g style={{ filter: 'drop-shadow(0 3px 3px rgba(120,100,40,0.22))' }}>
        <g transform={`translate(${x} ${y})`}>
          {dirs.map((deg, i) => (
            <path key={`u${i}`} d={`M0 0 Q ${s*0.5} ${-s*0.2} ${s} 0 Q ${s*0.5} ${s*0.2} 0 0 Z`}
                  fill="#3F8A52" transform={`rotate(${deg})`} />
          ))}
          {dirs.map((deg, i) => (
            <path key={`o${i}`} d={`M0 0 Q ${s*0.46} ${-s*0.15} ${s*0.9} 0 Q ${s*0.46} ${s*0.15} 0 0 Z`}
                  fill={i % 2 ? '#57A86A' : '#67B87A'} transform={`rotate(${deg + 22})`} />
          ))}
          <circle r={s*0.17} fill="#B98A56" stroke="#8A6438" strokeWidth="1.2" />
          <circle cx={-s*0.05} cy={-s*0.05} r={s*0.07} fill="#D6A668" />
        </g>
      </g>
    );
  }
